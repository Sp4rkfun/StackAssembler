...
public class StackContains extends Assertion {
	...
	public void runProcedure() {
		super.runProcedure();
		if(size!=State.stack.stackPointer()){
			Echo.printD("Size Mismatch.  Expected: "+size+" Actual: "+State.stack.stackPointer());
		}
		else{
			for(int i=0; i<size; i++){
				if(s[i].equals("?")) continue;
				if(Integer.parseInt(s[i])!=State.stack.stack.get(i)){
